...
class NeRFDataset:
    def __init__(self, opt, device, type="train", n_test=10):
        ...
        if "time" in frames[0]:
            ...
            print(
                f'[INFO] selecting time == 0 frames: {len(transform["frames"])} --> {len(frames)}'
            )
        ...
        if "fl_x" in transform or "fl_y" in transform:
            ...
            # blender, assert in radians. already downscaled since we use H/W - camera_angle_x/y is fov
            fl_x = (
        ...
        # We use inverse to specify a point in the other coordinate system.
        # If pose is extensic of camera, then we use inverse to transform a point from world to camera.
        # There fore x_c = torch.inverse(pose) @ x_w means transform x_w from world to camera. x_c is in camera's language
        # We then multiple the projection matrix with inversed pose to get the final projection matrix.
        # This can now be used ro prohect any point from world to clip space for any camera
    ...
    def collate(self, index):
        ...
        rays = get_rays(
            poses, self.intrinsics, self.H, self.W, num_rays, self.opt.patch_size
        )
