...
class NeRFRenderer(nn.Module):
    ...
    def refine_and_decimate(self):
        ...
        if self.bound <= 1:
            ...
            mesh = trimesh.Trimesh(v, f, process=False)
            mesh.export(
                os.path.join(self.opt.workspace, "mesh_stage0", "mesh_0_updated.ply")
            )
            ...
            for cas in range(self.cascade):
                ...
                cur_f = (
                    f[self.f_cumsum[cas] : self.f_cumsum[cas + 1]] - self.v_cumsum[cas]
                )
        ...
        print(
            f"[INFO] update stage1 mesh: {self.vertices.shape}, {self.triangles.shape}"
        )
    ...
    def export_stage0(
        ...
        if resolution == self.grid_size:
            ...
            if not self.opt.sdf:
                mask = torch.zeros(
                    [self.grid_size] * 3, dtype=torch.float32, device=device
                )
        ...
        **kwargs,
        ...
        if self.training:
            ...
            if self.opt.sdf:
                ...
                iter_cos = -(
                    F.relu(-true_cos * 0.5 + 0.5) * (1.0 - self.opt.cos_anneal_ratio)
                ...
                c = prev_cdf
                sigmas = (
                    ((p + 1e-5) / (c + 1e-5)).view(-1).clip(0, 1)
                )  # sigmas are alpha now

            weights, weights_sum, depth, image = raymarching.composite_rays_train(
                sigmas, rgbs, ts, rays, T_thresh, self.opt.sdf
            )
        ...
        **kwargs,
        ...
        if mask_flatten.any():
            with torch.cuda.amp.autocast(enabled=self.opt.fp16):
                mask_rgbs, masked_specular = self.rgb(
                    xyzs[mask_flatten]
                    if self.opt.enable_offset_nerf_grad
        ...
        alphas = (
            dr.antialias(
                ...
                vertices_clip,
                self.triangles,
                pos_gradient_boost=self.opt.pos_gradient_boost,
            )
            .squeeze(0)
            .clamp(0, 1)
        )
        ...
        if self.opt.ssaa > 1:
            ...
            trig_id = scale_img_hw(
                trig_id.float(), (h0, w0), mag="nearest", min="nearest"
            )
    ...
    def update_triangles_errors(self, loss):
        ...
        TORCH_SCATTER.scatter_add(
            torch.ones_like(values), indices, out=self.triangles_errors_cnt
        )
    ...
    def mark_untrained_grid(self, dataset, S=64):
        ...
        # This function marks the grid cells that are not covered by any camera. or outside of AABB. One of or both.
                ...
                for ys in Y:
                    for zs in Z:
                        ...
                        for cas in range(self.cascade):
                            ...
                            with torch.cuda.amp.autocast(enabled=self.opt.fp16):
                                sigmas = (
                                    self.density(cas_xyzs).reshape(-1).detach()
                                )
                                if self.opt.sdf:
                                    inv_s = torch.exp(self.variance * 10.0).clip(
                                        1e-6, 1e6
                                    )
